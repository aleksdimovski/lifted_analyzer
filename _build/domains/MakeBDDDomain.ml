...
module BDDAPRON_DOMAIN(Param: BDDAPRON_PARAM) : BDDDomain =
  struct
    ...
    let print fmt d =   (* Format.printf "%a@." (fun fmt x -> Bdddomain1.print fmt x) d.dom *)
		...
		List.iter (fun elem -> if !first then Format.fprintf fmt "(Bool: %a) && " (Expr1.Bool.print cond) (fst elem)
								else Format.fprintf fmt " || (Bool: %a) && " (Expr1.Bool.print cond) (fst elem);
